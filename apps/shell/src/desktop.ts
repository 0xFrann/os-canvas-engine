...
import type { DesktopBackground } from "./backgrounds";

/**
 * What an app can ask the desktop it runs on for. A small, explicit surface: an app fills a content
 * slot and reaches nothing else (ADR 004) — not the engine, not the DOM around its window — so a
 * setting that belongs to the whole desktop is handed to it here or not at all.
 *
